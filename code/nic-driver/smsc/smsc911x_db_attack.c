...
#define restore_reglist "pop {r14}\n\t"\
      "pop {r13}\n\t"\
      "pop {r12}\n\t"\
      "pop {r11}\n\t"\
      "pop {r10}\n\t"\
...
#define init_checksum         "ldr r1, [r0, #0] \n\t"\
      "ldr r2, [r0, #4]  \n\t"\
      "ldr r3, [r0, #8]  \n\t"\
      "ldr r4, [r0, #12] \n\t"\
      "ldr r5, [r0, #16] \n\t"\
...
#define save_checksum      "str r1, [r0, #0] \n\t"\
      "str r2, [r0, #4]  \n\t"\
      "str r3, [r0, #8]  \n\t"\
      "str r4, [r0, #12] \n\t"\
      "str r5, [r0, #16] \n\t"\
...
/*
//Checksum Function Pseudo-Code     
//#define RLP_ADDRESS           0x40000440
//#define RLP_SIZE              0x000027FC
Input : y number of iterations of the checksum function
Output: Checksum C
   daddr   : memory address to read
   x       : value of T function
   status  : status register
   PC      : program counter
   C[i]    : checksum[i]
   counter : counter of the number of iterations
Computation:
Initializing C[0-12] with challenges
for counter = y to 0
    //update x by T function
    x = x^2 or 5 + x
    //construct memory address to read
    //the entire swatt code is 8K Bytes
    //2^11 * 4 = 8K Bytes.
    //from 0x40000000 to 0x40002000 is verified by checksum function
    //from 0x40000000 to 0x4000043C is not swatt code
    offset = x & 0x00001FFC
    daddr = PC xor offset
    //update checksum C[i] based on
    //x, daddr, PC, status, n, and 4 other checksum states
    C[i] <-- C[i] + mem[daddr] xor C[i-2] + x xor PC xor C[i-1]+ daddr xor status
    or
    C[i] <-- C[i] xor mem[daddr] + C[i-2] xor x + PC + C[i-1] xor daddr + status
    rotation shift
    C[i] = rotation_right_shit_1_bits( C[i] )
    C[i] = C[i] + counter
    or
    C[i] = C[i] xor counter
    //combine X and C[i]
    x = x xor C[i] or x = x + C[i]
    //update i
    i = (i + 1) mod 12
...
register usage in DEBUG mode
r0  : debug buffer address
r1 - r8 : checksum states
r9  : mem[addr]
r10 : x (randomness) in T-func
r11 : tmp
r12 : counter
r13, r14 : not used
*/
...
void swattARM(int *challenge, int size)
{
     unsigned int addr;
...
void swattARM_fake_end(void)
{
	return;
}

static int smsc911x_do_swatt(struct net_device *dev, struct ifreq *ifr, int cmd)
{
  //  int ret = 0;
  int i = 0;
  unsigned int buf[64];
